...
- **Declarative step handlers** — `literal`, `model`, `transform`, `shell`; the dispatcher resolves `{ kind, config }` steps (backward-compatible with function steps). Routing is declarative, not learned.
